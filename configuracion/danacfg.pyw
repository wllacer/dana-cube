...
#        self.setCentralWidget(self.view)
    ...
    def test(self,index):
        print(self.model.item(index),self.model.columnCount(None))
        item = self.model.item(index)
        if item.type not in ITEM_ATTR:
            for col in range(len(ATTR_DICT)):
                self.view.showColumn(col +1)
                return
        for col in range(len(ATTR_DICT)):
            self.view.hideColumn(col +1)
            #self.view.resizeColumnToContents(col +1)
            if item.type in ITEM_ATTR:
                if ATTR_DICT[col] in ITEM_ATTR[item.type]:
                    self.view.showColumn(col +1)
