...
ITEM_ATTR = { 'cube':('name','table','base filter'),
               'conn':('driver','dbname','dbhost','dbuser','dbpass'),
               'fields': (),
               'guides':None,
               'favorites':None,
               'guide':('name','class','type'),
               'domain':('name','elem','filter','table','code','desc','grouped_by'),
               ...
               'join_clause':('rel_elem','base_elem','left','comparator','right'),
               'category':('default','result','condition','values'),
               'default':('cubo',),
               'vista':('row','col','agregado','elemento'),
               'favorite':('name',),
               #'date':('name','elem','type'),
              }
LIST_ATTR = ('elem','code','desc','grouped_by','values')
...
def load3(file):
    def append_elem(tipo,id,estructura,padre,ord=None):
        ...
        if isinstance(estructura,dict):
            for attr in estructura:
                if attr in ITEM_ATTR[tipo]:
                    if attr in LIST_ATTR:
                        #nclave ='{}{}{}'.format(clave,DELIMITER,attr)
                        #cubeTree.append(CubeItem('#entry',nclave,None,desc=attr,parent=cubeTree[clave]))
                        #valores = norm2List(estructura[attr])
                        #for ind,dato in enumerate(valores):
                            #print(attr,dato)
                            #mclave ='{}{}{}'.format(nclave,DELIMITER,ind)
                            #cubeTree.append(CubeItem('#entry',mclave,data=dato,desc=attr,parent=cubeTree[nclave],ord=ind))
                        nclave ='{}{}{}'.format(clave,DELIMITER,attr)
                        valores = norm2List(estructura[attr])
                        cubeTree.append(CubeItem('#ientry',nclave,data=valores,desc=attr,parent=cubeTree[clave]))

                    else:
                        nclave ='{}{}{}'.format(clave,DELIMITER,attr)
                        cubeTree.append(CubeItem('#entry',nclave,data=estructura[attr],desc=attr,parent=cubeTree[clave]))
                #else:
    ...
    def procesaGuias(definicion,clave_base):
        for ind,item in enumerate(definicion):
            ...
            clave_papa = append_elem('guide',clave,item,cubeTree[clave_base],ord=ind)
