...
class ItemEdit(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        linea = 0
        for k in range(0,len(ATTR_DICT),2):
            fieldLayout.addWidget(self.entradas[k][0], linea, 0)
            fieldLayout.addWidget(self.entradas[k][1], linea, 1, 1, 3)
            if k + 1 <len(ATTR_DICT):
                fieldLayout.addWidget(self.entradas[k+1][0], linea, 6)
                fieldLayout.addWidget(self.entradas[k+1][1], linea, 7, 1, 3)
            linea += 1
